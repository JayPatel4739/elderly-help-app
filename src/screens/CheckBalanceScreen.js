import React from 'react';
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, StatusBar, Image } from 'react-native';
import { colors } from '../constants/colors';

const CheckBalanceScreen = ({ navigation, t }) => (
  <View style={styles.container}>
    <StatusBar barStyle="dark-content" backgroundColor={colors.surfaceBright} />
    <View style={styles.topBar}><Text style={styles.accountIcon}>👤</Text><Text style={styles.topBarTitle}>{t('safePracticeMode')}</Text><Text style={styles.helpIcon}>❓</Text></View>
    <ScrollView contentContainerStyle={styles.contentContainer} showsVerticalScrollIndicator={false}>
      <View style={styles.balanceCard}>
        <View style={styles.balanceIcon}><Text style={styles.balanceIconText}>💰</Text></View>
        <View style={styles.balanceInfo}><Text style={styles.balanceLabel}>{t('yourBalance')}</Text><Text style={styles.balanceAmount}>{t('balanceAmount')}</Text></View>
        <View style={styles.verifiedBadge}><Text style={styles.verifiedIcon}>✓</Text><Text style={styles.verifiedText}>{t('securelyVerified')}</Text></View>
      </View>
      <View style={styles.successSection}><View style={styles.successCircle}><Text style={styles.checkmark}>✓</Text></View><Text style={styles.successText}>{t('accountInPracticeMode')}</Text></View>
      <View style={styles.heroImage}><Image source={require('../../assets/images/abstract-waves.jpg')} style={styles.heroImg} resizeMode="cover" /></View>
      <TouchableOpacity style={styles.homeButton} onPress={() => navigation.navigate('Home')} activeOpacity={0.8}><Text style={styles.homeIcon}>🏠</Text><Text style={styles.homeButtonText}>{t('backToHome')}</Text></TouchableOpacity>
      <View style={styles.footer}>
        <View style={styles.footerContent}><Text style={styles.footerIcon}>🛡️</Text><Text style={styles.footerTitle}>{t('noRealMoneyAtRisk')}</Text></View>
        <View style={styles.footerLinks}><Text style={styles.footerLink}>{t('safetyFirstLabel')}</Text><Text style={styles.footerDivider}>•</Text><Text style={styles.footerLink}>{t('practiceMode')}</Text></View>
      </View>
    </ScrollView>
  </View>
);

const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: colors.surfaceBright },
  topBar: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 24, paddingVertical: 16, backgroundColor: colors.surfaceBright, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 2 },
  accountIcon: { fontSize: 28 },
  topBarTitle: { fontSize: 24, fontWeight: '600', color: colors.onSurface },
  helpIcon: { fontSize: 28 },
  contentContainer: { paddingHorizontal: 24, paddingVertical: 20, paddingBottom: 40 },
  balanceCard: { backgroundColor: colors.primaryContainer, padding: 24, borderRadius: 16, alignItems: 'center', gap: 16 },
  balanceIcon: { backgroundColor: colors.surfaceContainerLowest, padding: 24, borderRadius: 48 },
  balanceIconText: { fontSize: 48 },
  balanceInfo: { alignItems: 'center', gap: 8 },
  balanceLabel: { fontSize: 18, fontWeight: '600', color: colors.onPrimaryContainer },
  balanceAmount: { fontSize: 32, fontWeight: '700', color: colors.primary },
  verifiedBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: `${colors.onPrimaryContainer}20`, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, gap: 8 },
  verifiedIcon: { fontSize: 16, color: colors.onPrimaryContainer },
  verifiedText: { fontSize: 18, fontWeight: '600', color: colors.onPrimaryContainer },
  successSection: { alignItems: 'center', paddingVertical: 32, gap: 16 },
  successCircle: { backgroundColor: colors.primaryContainer, padding: 16, borderRadius: 32 },
  checkmark: { fontSize: 36, color: colors.primary },
  successText: { fontSize: 20, color: colors.onSurface, textAlign: 'center', paddingHorizontal: 32 },
  heroImage: { height: 160, borderRadius: 16, overflow: 'hidden', marginVertical: 20 },
  heroImg: { width: '100%', height: '100%' },
  homeButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: colors.primary, height: 64, borderRadius: 16, gap: 12 },
  homeIcon: { fontSize: 24 },
  homeButtonText: { fontSize: 18, fontWeight: '600', color: colors.onPrimary },
  footer: { marginTop: 32, backgroundColor: colors.errorContainer, paddingVertical: 16, paddingHorizontal: 24, borderRadius: 16, alignItems: 'center' },
  footerContent: { flexDirection: 'row', alignItems: 'center', marginBottom: 8 },
  footerIcon: { fontSize: 20, marginRight: 8 },
  footerTitle: { fontSize: 18, fontWeight: '600', color: colors.error },
  footerLinks: { flexDirection: 'row', alignItems: 'center' },
  footerLink: { fontSize: 16, color: colors.onErrorContainer, opacity: 0.8 },
  footerDivider: { marginHorizontal: 16, opacity: 0.3 },
});

export default CheckBalanceScreen;
